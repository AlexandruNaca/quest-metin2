...
quest make_herb_lv10  begin
	...
	state information begin
		when letter begin
			...
			send_letter("&Lv 10 - Cercetarea biologului")
		...
		when __TARGET__.target.click or
			...
			say_title("Biologul Chaegirab:")
			say_verde("")																							
			say_verde("Momentan studiez Florile Kaki si am nevoie de")
	...
	state go_to_disciple begin
		...
		when button or info begin
			say_title("Colecteaza Flori kaki")
			say_verde("")
			say_verde("Biologul Chaegirab student al invatatorului Uriel")
			say_verde("tocmai cerceteaza Flori kaki.")
			say_verde("")
			say_verde("Vaneaza "..mob_name(177).." pentru a le obtine")
			say_verde("")
			say_item_vnum(50703) 
		...
		when 177.kill begin
			if pc.count_item(50703)<10- pc.getqf("collect_count")then
				pc.give_item2(50703, 1)
			end
		end
		
    	when 20084.chat."Ai Floare kaki?" with pc.count_item(50703) >0   begin
				say_title("Biologul Chaegirab:")
				say_verde("")
				...
				if s<= pass_percent  then
				   if pc.getqf("collect_count")< 9 then     --10 below
					...
					say_reward("in 30.000 experienta, 20.000 Yang.")
					say_reward("")	
