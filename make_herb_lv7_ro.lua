...
quest make_herb_lv7  begin
	...
	state information begin
		when letter begin
			...
			send_letter("&Lv 7 - Cercetarea biologului")
		...
		when __TARGET__.target.click or
			...
			say_title("Biologul Chaegirab:")
			say_verde("")
			say_verde("Momentan studiez Floriile clopotel si am nevoie")
	...
	state go_to_disciple begin
		...
		when button or info begin
			say_title("Colecteaza Flori clopotel")
			---                                                   l
			say_verde("")
			say_verde("Biologul Chaegirab student al invatatorului Uriel")
			say_verde("tocmai cerceteaza Floarile clopotel.")
			say_verde("")
			say_verde("Vaneaza "..mob_name(175).." pentru a le obtine.")
			say_verde("")
			say_item_vnum(50702) 
		...
		when 71035.use begin --confused syrup
			if pc.getqf("drink_drug")==1 then
				say_verde("")
				say_verde("A fost folosit o data.")
				say_verde("")
		...
		when 175.kill begin
			if pc.count_item(50702)< 10 -pc.getqf("collect_count") then
				pc.give_item2(50702, 1)
			end
		end
		
    	when 20084.chat."Ai Floare clopotel?" with pc.count_item(50702) >0   begin
				say_title("Biologul Chaegirab:")
				say_verde("")
				...
				if s<= pass_percent  then
				   if pc.getqf("collect_count")< 9 then     --10 below
					...
					say_reward("in 10.000 experienta, 15.000 Yang si")
					
					if pc.job==0 then
						pc.give_item2(11216)
						say_reward("Blindaj placa de fier+6")	
					end
					if pc.job==1 then
						pc.give_item2(11416)
						say_reward("Costum de fildes+6") 
					end
					if pc.job==2 then
						pc.give_item2(11616)
						say_reward("Platosa magica+6") 
					end
					if pc.job==3 then		
						pc.give_item2(11816)
						say_reward("Vesmant turcoaz+6") 
					end	
					pc.setqf("collect_count",0)
					pc.setqf("drink_drug",0)	
